...
export function AdminPanel() {
 ...
 const onLoginSubmit = async (data: LoginValues, isSuperAdminLogin: boolean = false) => {
    setIsLoading(true);
    setError(null);
    try {
        ...
        // Super Admin Check based on UID from .env file
        ...
        <AnimatePresence mode="wait">
            {view === 'login' && (
                <motion.div key="login" initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 50 }}>
                    ...
                    <CardContent>
                        <form onSubmit={loginForm.handleSubmit((data) => onLoginSubmit(data, false))} className="space-y-4">
                            <div>
                            ...
                            <div className="relative my-4">
                             ...
                             <Button type="button" variant="secondary" className="w-full" onClick={() => { setView('superadmin'); setError(null); }}>
            ...
            {view === 'superadmin' && (
                <motion.div key="superadmin" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }}>
                    <CardHeader>
                         <Button variant="ghost" size="sm" className="absolute top-4 left-4" onClick={() => { setView('login'); setError(null); }}>
                            <ArrowLeft className="mr-2 h-4 w-4" /> Back
                        </Button>
                        <CardTitle className="font-headline text-3xl tracking-tight text-center pt-10">Super Admin</CardTitle>
                        <CardDescription className="text-center">Enter your super admin credentials.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={loginForm.handleSubmit((data) => onLoginSubmit(data, true))} className="space-y-4">
                            <div>
                                <Label htmlFor="super-email">Email</Label>
                                <Input id="super-email" type="email" {...loginForm.register("email")} placeholder="superadmin@example.com" />
                                {loginForm.formState.errors.email && <p className="text-sm font-medium text-destructive">{loginForm.formState.errors.email.message}</p>}
                            </div>
                            <div>
                                <Label htmlFor="super-password">Password</Label>
                                <Input id="super-password" type="password" {...loginForm.register("password")} placeholder="********" />
                                {loginForm.formState.errors.password && <p className="text-sm font-medium text-destructive">{loginForm.formState.errors.password.message}</p>}
                            </div>
                            {error && <p className="text-sm font-medium text-destructive">{error}</p>}
                            <Button type="submit" className="w-full" disabled={isLoading}>
